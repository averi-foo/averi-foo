...
module.exports = (file) => {
	...
	if (ffmpegGifThumbnails && !firstFrameOnly) {
		...
		return new Promise((resolve, reject) => {
			ffmpeg(`${uploadDirectory}/unapproved/${file.filename}`)
				.on('end', () => {
					return resolve();
				})
				...
				.save(`${uploadDirectory}/unapproved/thumb/${file.hash}${file.thumbextension}`);
		});
	}
	...
	if (file.mimetype === 'image/webp') {
		...
		thumbnailing = im(`${uploadDirectory}/unapproved/${file.filename}[0]`);
	} else {
		thumbnailing = gm(`${uploadDirectory}/unapproved/${file.filename}${firstFrameOnly ? '[0]' : ''}`);
		if (!firstFrameOnly) {
	...
	return new Promise((resolve, reject) => {
		thumbnailing.resize(Math.min(thumbSize, file.geometry.width), Math.min(thumbSize, file.geometry.height))
			.write(`${uploadDirectory}/unapproved/thumb/${file.hash}${file.thumbextension}`, function (err) {
				if (err) {
					return reject(err);
				}
				return resolve();
