...
module.exports = {
	...
	replace: async (text, sourceBoard, map) => {
		...
		for (let i = 0; i < quotes.length; i++) {
			const oldQuote = quotes[i].substring(2);
			let newQuote;
			// oldQuote inside thread
			if (map[oldQuote]) {
				newQuote = `>>${map[oldQuote]}`;
			} else {
				// oldQuote to source board, outside moved thread
				newQuote = `>>>/${sourceBoard}/${oldQuote}`;
			}
			text = text.replace(new RegExp(`>>${oldQuote}`, 'g'), newQuote);
		}
		return text;
	},
