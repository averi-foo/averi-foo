...
module.exports = async (inputName, permissions, boardSettings, boardOwner, boardStaff, username, __) => {
	...
	if ((permissions.get(Permissions.MANAGE_BOARD_GENERAL) || !forceAnon) && inputName && inputName.length > 0) {
		...
		if (matches && matches.groups) {
			...
			// Anti-impersonation for me specifically lol. Temporary code for now, could get a library for this.
			if (!isBoardOwner && (name.normalize().replace("I", "l").toLowerCase().replace("|", "l").replace("а","a").replace("с","c").replace("п","n").replace("н","h").replace("Ι","l").replace("ι","l").replace("Ⅰ","l").replace(/\s/g, "") == boardOwner))
			{
