...
const { getInsecureTrip, getSecureTrip } = require(__dirname+'/tripcode.js')
	...
	, emojiRegexFunc = require(__dirname+'/emojiRegex.js');
...
module.exports = async (inputName, permissions, boardSettings, boardOwner, boardStaff, username, userId, maxLength, __) => {
	...
	if ((permissions.get(Permissions.MANAGE_BOARD_GENERAL) || !forceAnon) && inputName && inputName.length > 0) {
		...
		if (matches && matches.groups) {
			...
			if (groups.name) {
				...
				if (name != unidecode(name)) {
					...
					for (let i = 0; i < name.length; i++ ) {
						let char = name[i];
						const emojiMatch = sanitizedName.substring(i).match(emojiRegex);
						console.log(emojiMatch)
						
						// Detect emojis
						if (emojiMatch && emojiMatch.index === 0) {
							sanitizedName += emojiMatch[0];
							i += emojiMatch[0].length;
							continue;
						}
						
						// If all else fails, unidecode the char.
						sanitizedName += unidecode(char)
					}
			...
			// Anti-impersonation.
			if (!isBoardOwner && (
				(unidecode(name.toLowerCase()).replace(/\s/g, "") == boardOwner) ||
				(unidecode(name.toLowerCase()).replace("i","l").replace("s","c").replace("p","n").replace("|","l").replace("!","l").replace(/\s/g, "") == boardOwner)
			))
			{
