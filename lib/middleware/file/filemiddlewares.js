...
const { debugLogs } = require(__dirname+'/../../../configs/secrets.js')
	...
	, updateHandlers = () => {
		...
		['flag', 'emoji', 'banner', 'asset', 'post'].forEach(fileType => {
			...
			const fileNumLimitFunction = (req, res) => {
				const { __ } = res.locals;
				const isPostform = req.path.endsWith('/post') || req.path.endsWith('/editpost') || req.path.endsWith('/modpost');
...
module.exports = {
	...
	emoji: (req, res, next) => {
		return fileHandlers.emoji(req, res, next);
	},
	posts: (req, res, next) => {
