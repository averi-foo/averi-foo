		...
		const bulkWrites = triggerModes.map(p => {
			return {
				'updateOne': {
					...
					'update': {
						'$set': {
							/* reset=0 is "no change", the options go from 0-2, and get reset to 0 or 1 or 2.,
							so if >0, we subtract 1 otherwise no change */

							/* lockReset
							 0 = No change
							 1 = Unlock board
							 2 = Lock thread creation
							 3 = Unlock board for registered users only
							 */
